...
import { schoolBrand, schoolInitials, schoolLogoPath } from "../lib/schoolBrand";

import { listarProyectosDB, crearProyectoDB, eliminarProyectoDB, renombrarProyectoDB } from "../actions";
...
export default function DashboardHubPage() {
  ...
  const userSchool = String(user?.publicMetadata?.colegio ?? user?.publicMetadata?.school ?? "").trim();
  ...
  const saveEdit = async (id: string, e: React.MouseEvent) => {
    ...
    setSavingId(id);
    try {
      await renombrarProyectoDB(id, editNombre.trim(), editDesc.trim() || undefined);
      setEditingId(null);
      await refresh();
    } catch {
  ...
  const handleCreate = async () => {
    ...
    try {
      await crearProyectoDB(n, descripcion.trim());
      setNombre("");
      setDescripcion("");
      setOpenCreate(false);
      await refresh();
  ...
  const handleDelete = async (id: string, projectName: string) => {
    if (!confirm(`¿Estás seguro de que querés borrar "${projectName}"? Se eliminarán todas las encuestas asociadas irreversiblemente.`)) return;
    try {
  ...
  return (
    <div className="min-h-screen bg-[#F4F7FB] pb-12 font-sans overflow-x-hidden selection:bg-blue-200 text-slate-900">
      <header className={`sticky top-0 z-40 border-b px-6 py-4 backdrop-blur-xl shadow-sm shadow-blue-900/5 ${theme.ring} ${theme.bg}`}>
        <div className="mx-auto flex max-w-[1500px] items-center justify-between">
          <div className="flex items-center gap-6 lg:gap-8">
            <div className="flex items-center gap-3">
              ...
              <span id="fallback-apdes-text" className={`hidden font-display text-2xl font-black tracking-tight md:block ${theme.text}`}>
                {userSchool || "APDES"}
              </span>
            </div>
            ...
            <div className="relative">
              ...
              >
                {schoolLogo ? (
                  ...
                  <span className={`flex h-10 w-10 items-center justify-center rounded-full border bg-white text-xs font-black ${theme.ring} ${theme.text}`}>
              ...
              {isProfileOpen && (
                <div className="absolute right-0 mt-2 w-52 rounded-2xl border border-white bg-white/95 p-2 shadow-2xl backdrop-blur-xl">
                  {isAdmin && (
                    <Link href="/dashboard/admin" className="mb-2 flex w-full items-center justify-center rounded-xl border border-blue-200 bg-blue-50 px-4 py-2.5 text-sm font-black text-blue-700 transition-all hover:bg-blue-100">
                      Administrar perfiles
                    </Link>
                  )}
                  {isAdmin && (
                    <Link href="/dashboard/families" className="mb-2 flex w-full items-center justify-center rounded-xl border border-blue-200 bg-blue-50 px-4 py-2.5 text-sm font-black text-blue-700 transition-all hover:bg-blue-100">
                      Universo Familia
                    </Link>
                  )}
                  <SignOutButton>
      ...
      <main className="mx-auto mt-12 w-full max-w-[1500px] px-6">
          ...
          <div className="flex flex-col gap-2">
            ...
            {isDirector ? (
              <p className="text-sm font-medium text-slate-500 max-w-2xl">
                ¡Hola Director/a! Elegí un proyecto para entrar al panel. Primero revisá NPS general y participación familiar, después abrí la comparación de proyectos del año correspondiente (Mujeres/Jardín) para analizar evolución.
              </p>
            ) : (
              <p className="text-sm font-medium text-slate-500 max-w-xl">
                Seleccioná un espacio de trabajo para acceder a su panel de control, ver sus encuestas y realizar análisis avanzados.
              </p>
            )}
          </div>
            ...
            <div className="flex items-center gap-3">
              {compareMode ? (
                <>
                  ...
                  <button
                    onClick={() => { setCompareMode(false); setSelectedToCompare([]); }}
        ...
        {isAdmin && compareMode && (
          <div className="mb-6 flex items-center gap-3 rounded-2xl border border-indigo-100 bg-indigo-50/80 px-5 py-3.5">
            <ArrowsLeftRight size={18} weight="bold" className="text-indigo-500 shrink-0" />
            <p className="text-sm font-bold text-indigo-700">
              Modo comparación activo — hacé clic en los proyectos que querés comparar{" "}
              <span className="font-black">({selectedToCompare.length}/2 seleccionados)</span>
            </p>
          </div>
        )}
        ...
        {loading ? (
          <div className="flex h-64 w-full items-center justify-center rounded-[40px] border border-white bg-white/40 backdrop-blur-xl shadow-lg">
            <div className="flex flex-col items-center gap-4 text-blue-600">
              ...
              <p className="text-sm font-bold text-slate-500 uppercase tracking-widest">Cargando espacios...</p>
            </div>
          </div>
        ) : (
            ...
            {isAdmin && !compareMode && (
              ...
              >
                ...
                <div className="flex flex-col items-center gap-1">
                  <span className="font-black text-blue-700">Nuevo Proyecto</span>
                  <span className="text-xs font-medium text-blue-500">Crear base de datos</span>
                </div>
              </button>
            )}
                ...
                >
                  ...
                  <div>
                    <div className="flex justify-between items-start mb-4">
                      <div className={`flex h-12 w-12 items-center justify-center rounded-2xl shadow-inner transition-colors shrink-0
                        ${isSelected
                    ...
                    {editingId === p.id ? (
                      <div className="flex flex-col gap-2" onClick={(e) => e.stopPropagation()}>
                        <input
                          ...
                          onKeyDown={(e) => { if (e.key === "Enter") saveEdit(p.id, e as any); if (e.key === "Escape") cancelEdit(e as any); }}
                        ...
                        <div className="flex gap-2">
                          <button onClick={(e) => saveEdit(p.id, e)}
                            ...
                            {savingId === p.id ? "Guardando..." : <><Check size={13} weight="bold" /> Guardar</>}
                          </button>
                  ...
                  <div className="flex flex-col gap-3">
                    ...
                    {compareMode ? (
                      <p className={`text-sm font-bold ${isSelected ? "text-indigo-600" : "text-slate-400"}`}>
                        {isSelected ? `✓ Seleccionado (${selectedToCompare.indexOf(p.id) + 1})` : "Clic para seleccionar"}
                      </p>
                    ) : (
                      <Link
      ...
      {openCreate && (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
          <div className="absolute inset-0 bg-slate-900/40 backdrop-blur-sm transition-opacity" onClick={() => setOpenCreate(false)} />
          ...
          <div className="relative w-full max-w-[480px] scale-100 rounded-[32px] border border-white bg-white/80 p-8 backdrop-blur-2xl shadow-2xl shadow-slate-900/20 transition-all">
            <div className="flex items-start justify-between mb-6">
              <div className="flex flex-col gap-1">
                ...
                <h3 className="font-display text-2xl font-black tracking-tight text-slate-900">Crear proyecto</h3>
            ...
            <div className="space-y-4">
              <div>
                <label className="mb-1.5 block text-xs font-bold uppercase tracking-widest text-slate-500">Nombre del Proyecto</label>
                <input
                  value={nombre}
              ...
              <div>
                <label className="mb-1.5 block text-xs font-bold uppercase tracking-widest text-slate-500">Descripción (Opcional)</label>
                <textarea
                  value={descripcion}
